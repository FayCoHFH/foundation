...
5. The accepted V1 catalog reserves six code-owned typed singleton placement definitions: `HOME_HERO`, `HOME_FEATURED_STORY`, `HOME_FEATURED_NEWS`, `HOME_FEATURED_PROJECT`, `HOME_FEATURED_CAMPAIGN`, and `NEWS_FEATURED`. C4 implements the four Story/News keys; Project/Campaign slots await their typed domains. Each implemented key validates its legal target and public eligibility, can have a half-open active window, has audited history, and hides or uses its code-defined fallback when unavailable. `isFeatured` flags and a universal page builder are prohibited.
6. V1 homepage behavior is hybrid: the accepted catalog reserves Hero, Featured Story, Featured News, Featured Project, and Featured Campaign; C4 implements the first three as Story/News placements, while latest News and the participation/navigation/footer experience are composed from their authoritative sources. Project/Campaign slots and other placement definitions require their typed domains or a demonstrated editorial need.
