...
| P-07 | Accepted | The homepage uses structured hybrid curation: five manual singleton slots (Hero, Featured Story, Featured News, Featured Project, Featured Campaign), derived Latest News and Upcoming Events, and stable/configured domain sections. Featured content is a placement, not a distributed `isFeatured` flag or a page-builder setting. |
...
| A-05 | Accepted | Content Placement is a constrained typed subsystem with six code-owned singleton definitions: `HOME_HERO`, `HOME_FEATURED_STORY`, `HOME_FEATURED_NEWS`, `HOME_FEATURED_PROJECT`, `HOME_FEATURED_CAMPAIGN`, and `NEWS_FEATURED`. Each definition controls legal target types and safe fallback; active windows cannot overlap per key, ineligible targets cannot render, and no universal page builder is authorized. |
...
| M-05 | Accepted | Gate C (Communications Domain Product & Architecture Review) is complete. The recommended next assignment is **Slice 1 — Application Foundation and Scaffold**, which begins only when Sven explicitly authorizes it. |
