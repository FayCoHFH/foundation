...
- `PublicStorySubmission`: confidential intake, separate from `Story`, containing justified contact/relationship/story/acknowledgement data, privacy acknowledgment, triage-only sensitivity declarations, lifecycle status, bounded internal review notes, and the later private media/rights/evidence boundary. It has no public projection. Submission content and private source artifacts remain manually retained pending an approved schedule.
- An authorized editor explicitly converts an accepted submission into one new ordinary Story draft. The draft receives only the suggested title and story text as validated source material; the submitter's identity, public byline, consent, rights, clearance, evidence, and media are independently selected. A restricted one-to-one provenance record preserves source/version/actor evidence without entering public projections. Rejection, withdrawal, retention, and deletion follow the intake retention policy. A public News submission aggregate is not in V1.
...
- `MediaAsset`: metadata and lifecycle for an immutable sanitized public object: classification, storage key/store, checksum, MIME type determined server-side, byte size, dimensions, explicit credit treatment, creator/time, and derived rights state. Confidential Story Submission image rights remain outside this public asset model and are linked only through controlled promotion provenance.
- `PublicStorySubmissionMediaPromotion`: the one-time source bridge from a Ready private review derivative to a MediaAsset. It records source submission/media identity and version, actor/time, website baseline, explicit credit decision, and immutable clearance snapshots; it never copies the original, suggested credit, private key, or evidence.
- `MediaUsage`: a typed contextual join to an eligible editorial owner (Story revision, News revision, or Publication in the bounded C6B-3E scope) with use category and subject identity. Raw `SubmissionMedia` is not a MediaUsage; promotion creates no usage automatically. Each use re-evaluates current source rights/restrictions, and existing uses are surfaced for review rather than silently rewritten.
