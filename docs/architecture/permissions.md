...
- `media.upload`, `media.edit`, `media.rights.clear`, `media.public.use`, and `communications.media.promote`, with separate `media.private.read` and `media.private.manage` where private assets exist; confidential Story Submission clearance/evidence issue, process, list, delivery, replacement, and removal use `communications.submissions.review`, while restoring eligibility additionally requires `communications.media.restore_eligibility`. Promotion is explicit and limited to the sanitized derivative and baseline website use.
