...
| Public Story Submissions | Habitat platform, confidential intake boundary | Text-only `PublicStorySubmission`, separate from Story drafts; no media rows in C6B-1A; protected server-rendered review inbox/detail in C6B-2A/2B | Collect/use/retain only under the approved intake/privacy policy; later explicit conversion creates a new Story draft; administrative reads use allowlisted DTOs and no shared cache |
